...
class Partition:
    ...
    # Define adaptation functions
    def adapt_mutation_rate(generation):
        return initial_mutation_rate * (0.9 ** (generation / 10))  # Decrease by 10% every 10 generations

    def adapt_selection_power(generation):
        return initial_selection_power * (0.95 ** (generation / 20))  # Decrease by 5% every 20 generations
    ...
    def mutate(self):
        ...
        if flip_coin_m(0.5):  #self.m_rate
            # split a community
...
def pop_crossover_and_immigration(n_offspring, cross_rate):
    idx_to_cross = []
    as_is_offspring = []
    for i in range(n_offspring):
        idx1, idx2 = np.random.choice(len(pop), size=2, replace=False, p=PROBS)
        if flip_coin_m(0.5):  #cross_rate
...
# def elitist_selection_helper(combinations, diversity_scores):
...
def elitist_selection_helper(combinations, diversity_scores):
    ...
    for i, (idx1, idx2) in enumerate(combinations):
        similarity_result = results[i].get()  # Get the similarity result for the current combination
        diversity_score = diversity_scores[i]  # Get the diversity score for the current combination
        similarities_dict[tuple(sorted((idx1, idx2)))] = (similarity_result, diversity_score)
...
def elitist_selection(similarity_threshold, diversity_scores):
    index_batch_size = get_batch_size_for_n()
    combinations = list(itertools.combinations(range(index_batch_size), 2))
    similarities_between_solutions = elitist_selection_helper(combinations, diversity_scores)
    ...
    while len(elite_idx) < N_ELITE and idx < len(pop):
        ...
        for elite_indiv_idx in elite_idx:
            if (elite_indiv_idx, idx) not in similarities_between_solutions:
                ...
                similarities_between_solutions.update(elitist_selection_helper(new_combinations, diversity_scores))
            ...
            # print("***")
            # print("***")
            if jac[0] > similarity_threshold and diversity_score > 0.8:
                elite_flag = False
                break
        if elite_flag:
...
# def adapt_parameter(parameter, generation):
#     # Calculate a scaling factor based on elite fitness improvement
#     scaling_factor = max(0.5, 1.0 - (generation / MAX_GENERATIONS))
#
#     # Calculate new parameter value within the adaptive range
#     adapted_parameter = parameter * scaling_factor
#     adapted_parameter = max(min(adapted_parameter, 1.0),0.5)
#
#     return adapted_parameter

def adapt_parameter(parameter, generation):
    return parameter * (0.9 ** (generation / 10))  # Decrease by 10% every 10 generations

def calculate_diversity(population):
    diversity_scores = []
    for i, indiv_a in enumerate(population):
        div_score = 0
        for j, indiv_b in enumerate(population):
            if i != j:
                div_score += compute_partition_similarity(indiv_a.membership, indiv_b.membership)
        avg_div_score = div_score / (len(population) - 1)
        diversity_scores.append(avg_div_score)
...
def find_partition():
    ...
    initial_diversity_scores = calculate_diversity(pop)
    c_rate = 0.5

    for generation_i in range(1, MAX_GENERATIONS+1):
        start_time = time.time()
        # for indiv in pop:
        #     #print("mutation rate: ", indiv.m_rate)
        #     break
        ...
        diversity_scores = initial_diversity_scores  #calculate_diversity(pop)
        ...
        # elitist selection
        elite_idx = elitist_selection(elite_similarity_threshold, diversity_scores)
        ...
        offspring, immigrants = pop_crossover_and_immigration(n_offspring=POPULATION_SIZE-N_ELITE-N_IMMIGRANTS, cross_rate=c_rate)
        c_rate = adapt_parameter(c_rate, generation_i)
        # print("###")
        # print("crossover rate: ", c_rate)
        # print("###")
        ...
        for indiv in offspring:
            #print(indiv.m_rate)
            indiv.m_rate = adapt_parameter(indiv.m_rate, generation_i)
            # print(indiv.m_rate)
            # print("*******")
        ...
        #print("Generation", generation_i, "Top fitness: ",np.round(best_score, 6),"; Average fitness: ",np.round(np.mean(pop_fit), 6),"; Time per generation: ",np.round(time.time() - start_time, 2),"; convergence: ", cnt_convergence)
...
if __name__ == "__main__":
    ...
    print("Main parameter values: pop_size=", POPULATION_SIZE, "workers=",N_WORKERS, "max_generations=",MAX_GENERATIONS)
    mods = []
    for i in range(100):
        print("exp ", i)
        ...
        mods.append(best_partition.fitness)


    print(mods)
    print("Best modularity final: ", max(mods))
    np.save("TAU_partition_"+args.graph+".npy", best_partition.membership)
